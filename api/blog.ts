...
export const blogApi = {
  ...
  getBlogs: async (filters: BlogFilters = {}): Promise<BlogResponse> => {
    ...
    const response = await axios.get(`/api/v1/blog?${params.toString()}`);
    return response.data;
  },

  getBlogBySlug: async (slug: string): Promise<Blog> => {
    const response = await axios.get(`/api/v1/blog/slug/${slug}`);
    return response.data;
  },

  incrementViews: async (blogId: string): Promise<void> => {
    await axios.post(`/api/v1/blog/${blogId}/view`);
  },

  getPopularBlogs: async (limit: number = 5): Promise<Blog[]> => {
    const response = await axios.get(`/api/v1/blog/popular?limit=${limit}`);
    return response.data;
  },

  getRecentBlogs: async (limit: number = 5): Promise<Blog[]> => {
    const response = await axios.get(`/api/v1/blog/recent?limit=${limit}`);
    return response.data;
  },

  getRelatedBlogs: async (blogId: string, limit: number = 3): Promise<Blog[]> => {
    const response = await axios.get(`/api/v1/blog/related/${blogId}?limit=${limit}`);
    return response.data;
  },

  getAllTags: async (): Promise<string[]> => {
    const response = await axios.get('/api/v1/blog/tags');
  ...
  getAllBlogsAdmin: async (filters: BlogFilters = {}): Promise<BlogResponse> => {
    ...
    const response = await axios.get(`/api/v1/blog/admin/all?${params.toString()}`);
    return response.data;
  },
  ...
  updateBlog: async (blogId: string, data: BlogUpdate): Promise<Blog> => {
    const response = await axios.put(`/api/v1/blog/${blogId}`, data);
    return response.data;
  },
  ...
  generateSlug: async (title: string): Promise<{ slug: string }> => {
    const response = await axios.post(`/api/v1/blog/generate-slug?title=${encodeURIComponent(title)}`);
    return response.data;
  },
};
